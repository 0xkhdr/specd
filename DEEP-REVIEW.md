...
9. `yagni:` dual observability exporters — Prometheus text format (`core/prometheus.go`, 171 lines) **and** OTel JSON (`adapter/otel_export.go`, 65 lines) for a local, single-shot CLI. Both are reachable via `report`, but two metric schemas for a tool with no long-running process is speculative. Pick one, document it in `docs/observability.md`, delete the other. [internal/core/prometheus.go, internal/adapter/otel_export.go]
