...
- [x] `.specd/harness/` layout + `harness.json` manifest (name, version,
  provenance) parse/validate.
- [x] `specd harness push/pull <git-url>` via stdlib-exec git (scrubbed env,
  URL validation); pull refuses local-modification overwrite without
  `--force`.
- [x] Quarantine: imported `command` checks disabled + listed; explicit
  per-item enable recorded in decision log; tests.
- [x] Round-trip e2e vs local bare-repo fixture; hostile manifest fixtures.
- **Validation:** `go test ./internal/core/... ./internal/cmd/... -run Harness -race -count=2`

## Wave 2 — Unified dashboard (P6.2, depends on Wave 1)

- [x] Extend `specd serve`: server-rendered views for conductor sessions,
  waves, eval trends, cost, escalations from state/ledgers; SSE reuse; no
  new deps.
- [x] `specd dashboard` alias with `--mode` filter.
- [x] Deterministic render-from-fixtures tests; no-outbound-network
  assertion.
- **Validation:** `go test ./internal/cmd/... -run 'Serve|Dashboard' -race`

## Wave 3 — Pack registry (P6.3, depends on Wave 1)

- [x] Registry index resolution (git repo index); `init --pack <git-url|name>`.
- [x] Checksum pinning lockfile; mismatch → hard fail; quarantine rules
  shared with Wave 1.
- [x] Remote pack e2e vs local fixture repo.
- **Validation:** `go test ./internal/pack/... -run Registry -race -count=2`

## Rollout & cleanup

- [x] Docs: user-guide harness-sharing section, command-reference
