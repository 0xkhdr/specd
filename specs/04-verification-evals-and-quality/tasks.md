...
| id | role | files | depends-on | verify | acceptance |
|---|---|---|---|---|---|
| [x] T07 | craftsman | internal/core/eval_import.go; internal/core/eval_import_test.go; internal/core/eval_store.go; internal/core/eval_store_test.go | T04,T06 | go test ./internal/core -run 'Test(EvalImport|EvalStore)' | JSON/JSONL schema/digest/task/check/provenance validation; stable findings; no network R3 |
...
| [x] T09 | craftsman | internal/orchestration/trace.go; internal/orchestration/trace_test.go; internal/orchestration/acp.go; internal/orchestration/acp_test.go | T04 | go test ./internal/orchestration -run 'Test(Trace|ACP)' | normalized observable V1 events, monotonic sequence/unique ids/sanitized field rejection R4 |
| [x] T10 | craftsman | internal/core/trajectory.go; internal/core/trajectory_test.go; internal/core/eval_import.go; internal/core/eval_import_test.go | T07,T09 | go test ./internal/core ./internal/orchestration -run 'Test(Trajectory|EvalImport|Trace)' | required/forbidden tool policy and trace digest validation; no hidden reasoning R4 |
