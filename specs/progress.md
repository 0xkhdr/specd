...
| ⬜ T1.2 | 01 | `go.mod` | zero `require` deps _(prose, not runnable — needs a real verify)_ |
...
| ⬜ T1.3 | 01 | `main.go`, `internal/cli/args.go` | bare invocation lists 16 verbs _(prose, not runnable — needs a real verify)_ |
| ✅ T1.4 | 01 | `commands_test.go` | `go test ./internal/core -run TestRegistryMatchesHelp` |

### Wave C — lifecycle & parser (parallel) ✅
| task | domain | files | verify |
|---|---|---|---|
| ✅ T2.4 | 02 | `internal/cmd/lifecycle.go` (consolidated) | `go run . new demo && test -f .specd/specs/demo/state.json` |
| ✅ T2.5 | 02 | `internal/cmd/lifecycle.go` (consolidated), `internal/core/task_complete.go` | `go test ./internal/cmd -run TestApproveGates` |
| ⬜ T2.6 | 02 | `internal/cmd/registry.go` (consolidated) | `go run . status demo --json \| grep '"mode":"simple"'` |
| ✅ T2.7 | 02 | `internal/core/state_test.go` (consolidated) | `go test ./internal/core -run TestSaveStateRequiresLock` |
...
| ✅ T5.1 | 05 | `internal/core/verify/exec.go` (customgate consolidated) | `go test ./internal/core/verify -run TestScrubbedEnv` |
| ⬜ T5.2 | 05 | `internal/core/evidence.go` (consolidated) | `go test ./internal/core/evidence -run TestAppendOnly` _(package path stale — no such package/test)_ |
| ✅ T5.3 | 05 | `internal/core/verify/exec.go` (capture consolidated) | `go test ./internal/core/verify -run TestChangedFiles` |

### Wave D — gates, evidence integrity, dispatch ✅
| task | domain | files | verify |
|---|---|---|---|
| ✅ T3.1 | 03 | `gates/registry.go` | `go test ./internal/core/gates -run TestRegistryOrder` |
| ✅ T3.2 | 03 | `internal/core/gates/core.go` (ears consolidated), `internal/core/dag.go` | `go test ./internal/core/gates -run TestCoreGates` |
| ✅ T3.3 | 03 | `internal/cmd/registry.go` (consolidated) | `go run . check demo` |
| ✅ T5.4 | 05 | `task_complete.go` | `go test ./internal/core -run TestCompleteRequiresEvidence` |
| ⬜ T5.5 | 05 | `internal/cmd/registry.go` (consolidated) | `go run . verify demo T1` |
| ✅ T5.6 | 05 | `internal/cmd/registry.go` (consolidated) | `go test ./internal/cmd -run TestRevertOnFail` |
| ✅ T5.7 | 05 | `verify/sandbox_test.go` | `go test ./internal/core/verify -run TestSandboxFailClosed` |
| ✅ T4.4 | 04 | `internal/cmd/registry.go` (consolidated), `internal/core/frontier.go` | `go run . next demo --json` |
| ✅ T4.5 | 04 | `internal/cmd/registry.go` (consolidated) | `go run . next demo --waves` |
...
| ✅ T8.3 | 08 | `internal/context/manifest.go` (consolidated) | `go build ./... && go vet ./...` |
| ⬜ T8.4 | 08 | `internal/cmd/registry.go` (consolidated) | surfaces share the engine (diff) _(prose, not runnable — needs a real verify)_ |
...
| ✅ T6.5 | 06 | `internal/integration/snippet.go` | `go test ./internal/integration -run TestAdapterConformance` |
| ✅ T6.6 | 06 | `internal/core/roles.go` | `go test ./internal/core -run TestRolePromptDedup` |
...
| task | domain | files | verify |
|---|---|---|---|
| ✅ T7.1 | 07 | `internal/mcp/server.go`, `internal/cmd/registry.go` (consolidated) | `echo '{...tools/list}' \| go run . mcp` |
...
| ✅ T11.2 | 11 | `internal/cmd/registry.go` (consolidated) | `go run . status demo --json` |
...
| ✅ T11.5 | 11 | `internal/core/report_test.go` (consolidated) | `go test ./internal/core -run TestNoLLMInRender` |
...
| ✅ T9.9 | 09 | `internal/orchestration/authority.go` | `go test ./internal/orchestration -run TestFailClosedAuthority` |
