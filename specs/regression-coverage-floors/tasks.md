...
- [x] Run `make cover-check`; record measured % vs. floor per package. File:
  `scripts/coverage-check.sh`.
- **Validation:** `make cover-check`

## Wave 2 — Ratchet (depends on Wave 1)

- [x] For each package with ≥1% headroom over its floor, raise the floor one step
  toward the `TESTING.md` target. File: `scripts/coverage-check.sh`.
- [x] Leave thin-headroom packages (core/cmd/worker/mcp/harness) unchanged to
  avoid noise flaps.
- **Validation:** `make cover-check`

## Wave 3 — Guard (depends on Wave 2)

- [x] Confirm CI `coverage-floor:` job passes with the raised floors (local `make ci`; remote CI pending).
- **Validation:** `make cover-check` (locally) → CI green

## Rollout & cleanup

- [x] Delete stray `coverage-*.out` artifacts from the repo root if regenerated.
