...
### [x] W4.1a — `brain resume` subcommand
...
### [x] W4.4b — Deterministic cost-brake test
...
### [x] W4.2a — Document the adapter-addition pattern
...
- [x] `brain resume --session <id>` first-class + tested (incl. no double-dispatch).
- [x] Cost brake: warn@80 / halt@100, deterministic synthetic-report test.
- [x] Opt-in metrics format (Prometheus/OTLP-JSON), golden-fixture tested, no new dep.
- [x] Adapter pattern + `--config` fallback documented.
- [x] Update `specs/progress.md` W4 + program exit criteria.
