...
- [x] **T1 — Fix `BuildProgram` nil-state TOCTOU deref**
...
- [x] **T2 — Test: `BuildProgram` with a spec whose state.json is missing**
...
- [x] **T3 — Harden `io.go` error paths (`AtomicWrite`, `AppendFile`)**
...
- [x] **T5 — Test: `LoadProgram`/`SaveProgram` round-trip + corrupt input**
...
- [x] **T6 — Test: DAG `RunnableFrontier`/`GroupWaves`/`WaveViolations`**
...
- [x] **T7 — Test: `DetectCycle` path content + `CriticalPath` invariant**
...
- [x] **T8 — Test: `LintEars` acceptance-criteria state machine**
...
- [x] **T9 — Test: `MergeSection`/`MergeAgentsMD` idempotency & marker edges**
...
- [x] **T12 — Raise coverage floors to new measured baseline**
...
- [x] **T13 — Extract `requireRootAndSlug` command prologue helper**
...
- [x] **T18 — Add `govulncheck` + `staticcheck` jobs and `.golangci.yml`**
  - role: coder · depends: — · requirements: R4.1
  - verify: `go run golang.org/x/vuln/cmd/govulncheck@latest ./...`

- [x] **T19 — Pin floating action refs + add `dependabot.yml`**
...
- [x] **T20 — Release workflow runs full `make ci`**
  - role: coder · depends: — · requirements: R4.3
  - verify: `N/A` — complete with `--unverified --evidence "<release.yml diff>"`

- [x] **T21 — Add `toolchain` directive + Go version matrix on test job**
...
- [x] **T22 — Fix AGENTS.md/TESTING.md boot/enrich drift**
...
- [x] **T27 — godoc breadth, stress.sh python3 warning, goreleaser SBOM**
