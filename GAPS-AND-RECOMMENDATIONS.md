...
| P1 | Resolved | The tracked remediation analysis now opens with a historical warning, pins commit `2ccd2a6`, and directs agents to `specd help --json` and the current command reference (`AGENT-DRIVEABILITY-ANALYSIS.md:3-8`). `scripts/docs-lint.sh` enforces that scope marker without rewriting the preserved findings. | An agent treating the historical analysis as current operating instructions could infer obsolete paths or evidence producers. | Keep the historical scope lint armed and preserve the underlying audit record unchanged. |
...
| P2 | Resolved | The eval probe pipes CLI output through a structural Go assertion helper (`internal/testutil/evalstatusassert/main.go`) that decodes the report, validates the envelope, and compares class, producer, and schema against canonical `internal/core` constants. | Formatting-only JSON changes no longer fail the semantic regression, while contract drift still fails closed. | Keep the helper dependency-free and tied to canonical eval constants. |
