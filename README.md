...
- 🛡️ **Validation Gates**: Programmatic checks (`specd check`) — 15 core gates (EARS syntax, design sections, task schema, acyclic DAG, evidence, sync, context budget, and more — 12 always-on plus opt-in `criteria` and `review`) plus a separate opt-in security gate.
