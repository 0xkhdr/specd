...
- 🛡️ **Validation Gates**: Programmatic checks (`specd check`) — 17 core gates (EARS syntax, design sections, task schema, acyclic DAG, evidence, sync, context budget, and more — 14 always-on plus opt-in `criteria` and `review`) plus a separate opt-in security gate.
