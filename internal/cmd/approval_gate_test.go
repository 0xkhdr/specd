...
func TestNextGatedOnApproval(t *testing.T) {
	...
	// Approving requirements records the gate but leaves status (and therefore
	// phase) at requirements/perceive, so the phase gate still fails closed —
	// requirements approval alone does not unlock execution verbs.
	if err := Run(root, "approve", []string{"demo", "requirements"}, nil); err != nil {
		t.Fatalf("approve requirements: %v", err)
	}
	if err := Run(root, "next", []string{"demo"}, map[string]string{"json": "1"}); err == nil {
		t.Fatalf("next after requirements-only approval succeeded (want phase-gate rejection)")
	}

	// Approving design advances status to design (analyze phase), clearing both
	// the phase gate and the handler's task-approval gate.
