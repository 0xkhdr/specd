...
func TestSecurityApproveAndRevokeException(t *testing.T) {
	...
	if err := Run(root, "exception", []string{"approve", "fp"}, flags); err != nil {
		t.Fatal(err)
	}
	...
	flags["action"] = "revoke"
	if err := Run(root, "exception", []string{"revoke", "fp"}, flags); err != nil {
		t.Fatal(err)
	}
...
func TestSecurityApproveCannotWaiveEvidence(t *testing.T) {
	...
	if err := Run(t.TempDir(), "exception", []string{"approve", "evidence-integrity"}, flags); err == nil {
