...
var Commands = []CommandMeta{
	...
	},
	{
		...
		Flags:           []FlagMeta{{Name: "criterion", Type: "string"}, {Name: "status", Type: "string"}, {Name: "evidence", Type: "string"}, {Name: "revert-on-fail", Type: "boolean", Description: "On a failed verify, stash the working tree (recoverable) instead of leaving it dirty"}},
	...
	},
	{
		...
		Description: "Stream runnable-frontier changes as NDJSON",
		Usage:       "specd watch [--once] [--spec <slug>]", Synopsis: "specd watch [--once] [--spec <slug>]",
		LongDescription: "Watches spec state and emits a newline-delimited JSON FrontierEvent whenever a spec's runnable task set changes. Read-only: it never writes state. --once does a single pass and exits; otherwise it polls at SPECD_WATCH_INTERVAL_MS (default 1000ms).",
		Flags:           []FlagMeta{{Name: "once", Type: "boolean", Description: "Emit current frontiers once and exit"}, {Name: "spec", Type: "string", Description: "Limit the stream to a single spec slug"}},
		ExitCodes:       []ExitCodeMeta{{0, "Success"}, {2, "Usage error"}, {3, ".specd/ not found"}},
		Examples:        []string{"specd watch --once", "specd watch --spec my-feature"},
	},
	{
